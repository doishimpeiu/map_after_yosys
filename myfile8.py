...
def can_retiming(example_node):
  ...
  # if len(upper_wires) != 2: #マルチプレクサの時はここの値を変えたものを作って後で統合する
  #   print("エラー出てもうてます")
  #   print(upper_wires)
  #   input()
  ####上をみる
  for upper_wire in upper_wires:
    ...
    if len(list(g.successors(upper_wire))) != 1: #ワイヤーが2つ以上のノードに繋がっている時
      # print(upper_wire)
      # if upper_wire == "__substreamoutput_data_861":
      #   print(upper_wire)
      #   input()
      # input()
      return False
    if len(list(g.predecessors(upper_wire))) != 1:
      print("複数入力")
      print(list(g.predecessors(upper_wire)))
      input()
      return False
    upper_nodes = list(g.predecessors(upper_wire))
    print(f"upper_nodes:{upper_nodes}")
    if len(upper_nodes) != 1: #マルチプレクサの時はここの値を変えたものを作って後で統合する
      print("エラー出てもうてます2")
      print(f"upper_nodes:{upper_nodes}")
      input()
      return False
...
def main():
  ...
  ret_count = 0
  res_list = []
  # example_node = add_list[0]
  # res = can_retiming(example_node)
  # print(res)
  index = 0
  for example_node in add_list:
    ...
    if res == False:
      ...
      # print("Falseだったので止めました")
      # print(example_node)
      # input()
    elif type(res) == str: #帰ってきた値がstr型ということは, 下のノードがadd, mul, muxまたはそれ以外
      print(res)
      # input()
      if res == "else": #帰ってきた値がadd, mul, muxではない場合
        print("変なの帰ってきてもうてます")
        print(res)
        input()
      else:
        print("retiming2が適用できる")
        print(res)
    elif res == True: #リタイミング条件に合っているので, リタイミングしていくぅ!
      print(f"{example_node}をリタイミングしていきます")
      # print(type(example_node))
